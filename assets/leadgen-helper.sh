...
cmd_daily_sends_count() {
  ...
  if [[ -d "$LEADS_ACTIVE" ]]; then
    for f in "${LEADS_ACTIVE}"/*.json; do
      [[ -f "$f" ]] || continue
      c=$(grep -c "\"sent_date\": *\"${today}" "$f" 2>/dev/null || true)
      count=$((count + ${c:-0}))
    done
  fi
...
cmd_domain_sends_count() {
  ...
  if [[ -z "$one_hour_ago" ]]; then
    echo "ERROR: could not compute rate-limit window (date). Failing closed: treat as limit reached." >&2
    return 1
  fi
  ...
  if [[ -d "$LEADS_ACTIVE" ]]; then
    for f in "${LEADS_ACTIVE}"/*.json; do
      ...
      c=$(grep -c "\"sent_date\": *\"${one_hour_ago}.*\".*@${safe_domain}" "$f" 2>/dev/null || true)
      count=$((count + ${c:-0}))
    done
  fi
...
cmd_write_email_body() {
  # Write email body to a private, unpredictable temp file inside the workspace.
  # Never use a predictable /tmp path: shared hosts allow pre-creation/symlink
  # attacks and world-readable lead content.
  local tmp_dir="${LEADGEN_DIR}/.tmp"
  mkdir -p "$tmp_dir"
  chmod 700 "$tmp_dir"
  local tmp_file
  tmp_file=$(mktemp "${tmp_dir}/email_body.XXXXXX") || { echo "ERROR: mktemp failed" >&2; return 1; }
  chmod 600 "$tmp_file"
